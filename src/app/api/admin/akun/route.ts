...
export async function POST(req: NextRequest) {
  ...
  const { tipe, targetId, nim: nimInput, email, password } = await req.json();
  ...
  const duplicate = await prisma.user.findFirst({ where: { OR: [{ nim }, { email }] } });
  if (duplicate) return NextResponse.json({ message: "NIM atau email telah digunakan akun lain." }, { status: 400 });
  const user = await prisma.user.create({ data: { name: target.nama, nim, email, password: await bcrypt.hash(password, 10), role: tipe } });
